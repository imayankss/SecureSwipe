...
The optional SQLite durability prototype and synthetic order-integrity
reference sit outside the API request path. Neither is enabled by default, and
neither contributes to Lane A, Lane B, or local-serving fraud metrics.
...
The checked-in deployment units are the static Next.js frontend and a separate
FastAPI container target. The service image excludes datasets, model bundles,
reports, notebooks, frontend sources, tests, and credentials; an operator must
mount a reviewed bundle separately. This packaging boundary does not establish
a public backend or production deployment.
...
## Reference-only scale shape

The PostgreSQL substrate implements shared idempotency and one transactional
audit chain for the explicit `postgres-scale` profile. Its local multi-process
tests establish correctness boundaries only; they do not prove a deployable
replica fleet, public availability, or throughput scaling.

```mermaid
flowchart LR
  Client["Authorized client"] --> LB["Load balancer\nreference only"]
  LB --> R1["API replica\nreference only"]
  LB --> R2["API replica\nreference only"]
  R1 --> PG["PostgreSQL idempotency\nand audit state"]
  R2 --> PG
  R1 --> Bundle["Verified read-only bundle\ndistribution required"]
  R2 --> Bundle
  R1 --> Queue["Human-review queue\nreference only"]
  R2 --> Queue
  PG --> Monitor["Hosted monitoring and alerting\nreference only"]
```

The load balancer, replica supervision, shared bundle distribution, public
authentication, TLS and rate-limit ownership, durable human-review queue, and
hosted monitoring in this sketch are not implemented. A feature store is also
absent because current request contracts supply features directly. Any future
benchmark must remain tied to its exact source SHA, hardware, worker count,
concurrency, and evidence category rather than becoming a universal capacity or
Razorpay-scale claim.
