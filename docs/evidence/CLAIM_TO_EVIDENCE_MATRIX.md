...
| 5.8 | Under synthetic fault injection at eight pre-registered crash points, **no fault produced an approval-like or below-threshold-success response**. Corruption, truncation, mutation, an unwritable audit sink, and a missing model fingerprint all fail closed, two of them by refusing to start. | `docs/evidence/MT6_STATE_AND_CRASH_DECISION.md`; `tests/test_operations_state_recovery.py` | `VERIFIED` / `SYNTHETIC`. 16 deterministic fault-injection tests using injected hooks, not timing sleeps. Local single-process scope only |
| 5.9 | **Demonstrated gap in the in-memory design:** a crash after the audit event is durable, or any process restart, releases the idempotency key. A duplicate retry then rescores, returns `200`, and writes a second audit event sharing a `request_id` — after which the chain never verifies again and the service cannot reopen its own audit log. | `docs/evidence/MT6_STATE_AND_CRASH_DECISION.md` §4 | `VERIFIED` / `SYNTHETIC`. Proven at fault points 4b, 5 and 6. Fail-closed rather than wrong, but it converts a recoverable retry into total unavailability |
| 5.10 | An **optional** local SQLite idempotency prototype closes those gaps for the single-node case: completed requests replay after restart without rescoring, and unresolved work fails closed instead of being rescored. The in-memory registry remains the default and the API is unchanged. | `src/operations/durable_idempotency.py`; `tests/test_operations_durable_idempotency.py`; protocol SHA-256 `d2911e8c3b116a43a7395febc2e06b0647ca42002db82cb0df08f20d737b1078` | `VERIFIED` / `SYNTHETIC`. **Local single-node durability only** — not immutable/WORM, not ACID across services, not multi-writer, not high availability, not cross-host failover. Stores only hashed key, input digest, lifecycle state, allowlisted decision label, model version, audit event hash and timestamps; database bytes were inspected directly with synthetic sentinels |
| 5.11 | A synthetic, deterministic **pre-model order-integrity reference** demonstrates server-side amount reconstruction: the client may send only a SKU and a quantity, and every amount is recomputed from a trusted synthetic catalog in integer minor units. | `docs/evidence/MT7_ORDER_INTEGRITY_DECISION.md`; `src/order_integrity/verifier.py`; protocol SHA-256 `617b06fcc1cb792baadb8b96d671f7f49f383be99c746fae5c911c5a60afb05d` | `VERIFIED` / `SYNTHETIC`. **Not a live Razorpay integration, not evidence about any real incident, and not part of fraud-model metrics.** Four allowlisted outcomes, none named approved/accepted/fraudulent. Never wired into `/v1/predict` |
| 5.12 | Across 300 generated synthetic carts, an injected client price-like field is always refused and never influences the server-reconstructed total. | `tests/test_order_integrity_verifier.py` property test | `VERIFIED` / `SYNTHETIC`. Demonstrates the single lesson: never trust a client-supplied total. The property test also caught a real per-line vs merged-per-SKU quantity bypass, which was fixed in the contract |
| 5.13 | A read-only Razorpay context adapter is **deferred and not implemented**. No Razorpay API, MCP, SDK, webhook, key, secret, credential, or external payment action is used anywhere in this repository. | `docs/evidence/EXECUTION_LEDGER.md` MT8 entry | `FUTURE OR DEFERRED — NOT IMPLEMENTED`. Blocked on owner-supplied safe test credentials and a verified field mapping. Absence of an integration is stated as fact; no capability is implied |

**MT7 order-integrity boundary:** rows 5.11-5.12 describe a synthetic
pre-model reference design. It is not a live Razorpay integration, not a real
payment workflow, not evidence about any real incident, not production-ready,
not PCI-relevant, and not a second claimed loss class. It contributes nothing to
average precision, ROC-AUC, precision, recall, Brier score, calibration,
capacity tiers, or the Lane A cost explorer.

**MT6 state boundary:** rows 5.8-5.10 describe local, single-process state
behaviour under synthetic fault injection. They are not production durability,
ACID across services, immutability, high availability, multi-region resilience,
multi-replica safety, or a Razorpay integration, and MT6 makes no performance
claim of any kind.
...
| 6A.10 | A static, client-side explorer makes the Lane A capacity trade-off inspectable: raising illustrative capacity from 100 to 2,000 reviews/day raises recall from 27.18 % to 93.84 % and simultaneously raises legitimate transactions sent to human review from 2,239 to 58,663. | `docs/evidence/MT5_COST_EXPLORER_EVIDENCE.md`; `web/components/dashboard/LaneACostExplorer.tsx`; contract SHA-256 `38258fb9c55dd476cd3adb4beb35875b84f66e455032a19df355bcece15d2d94` | `VERIFIED` / `CURRENT/MEASURED`. Counts are the sealed final aggregates, unmodified. A false positive is a legitimate transaction sent to human review, never an automatic decline |
| 6A.11 | Illustrative merchant cost is computed as `(TP + FP) × review cost + FP × friction cost + FN × (missed-fraud loss + chargeback cost)`, with all four INR assumptions editable and visible. | `docs/evidence/MT5_COST_EXPLORER_CONTRACT.md`; `web/lib/laneACostModel.ts`; `web/__tests__/lane-a-cost-model.test.tsx` | `VERIFIED` / `SYNTHETIC`. **Illustrative scenario only.** The model computes no savings, ROI, net benefit, or optimal operating point, and selects no capacity or threshold. Assumptions are illustrative starting values, not merchant data, defaults, or recommendations |
| 6A.12 | Which capacity tier looks cheapest is an artefact of the chosen assumptions, not a property of the model: tripling the review cost and tripling the missed-fraud loss reorder the tiers differently. | `docs/evidence/MT5_COST_EXPLORER_EVIDENCE.md` §5 | `VERIFIED` / `SYNTHETIC`. Two predeclared sensitivity scenarios are shown side by side with no winner highlighted. These are scenario calculations on published aggregate counts, not observed merchant costs |

**Lane A cost-explorer boundary:** rows 6A.10-6A.12 are illustrative scenario
arithmetic. They are not Razorpay economics, merchant pricing, savings, ROI,
cost reduction, fraud prevented, production readiness, a recommended capacity,
or an optimal threshold, and they are never compared with Lane B.
